...
static int verrify_4(int size[2], int x, int y, char c)
{
    int l = size[0];
    int h = size[1];
...
static int is_correct_4(char *str, int size[2])
{
    int l = size[0];
    int h = size[1];
    int error = 0;
    ...
    for (int y = 0; y != h; y++)
        for (int x = 0; x != l + 1; x++)
            error += verrify_4(size, x, y, str[x + (l + 1) * y]);
    if (error != 0)
        return 84;
    return 0;
}
