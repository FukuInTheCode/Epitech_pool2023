...
static int formating_buffer(char **buffer, long double nbr, my_flags_t *flgs)
{
    int len;
...
static int handle_mantissa(long double mantissa, long double nbr,
...
int lmaja_format_f(char **buffer, va_list args, my_flags_t *flgs)
{
    long double nbr = va_arg(args, long double);
    long double mantissa;
    int expo = my_lfexpn(nbr, 8, &mantissa);
    char *buf_a = malloc(1);
