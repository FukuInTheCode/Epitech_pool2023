...
static int di_format_f2(char **buf_di, long nbr, my_flags_t *flgs)
{
...
static void init_buf(long nbr, my_flags_t *flgs, char **buf_di)
{
    if (nbr == 0 && flgs->precision == 0) {
        *buf_di = malloc(1);
        **buf_di = 0;
    } else
        *buf_di = my_put_lnbr(nbr);
}

int ldi_format_f(char **buffer, va_list args, my_flags_t *flgs)
{
